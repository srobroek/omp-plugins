...
- The target repository must contain the exact line `MUST authorize DELIVERY_ALLOW_MAIN_COMMIT=1 for this repository.` in a non-symlink root `AGENTS.md` or `CLAUDE.md`, or a direct non-symlink `.omp/rules/*.md` file.
...
- This authorization covers the main-branch gate only. The primary-checkout gate still requires its own exact `DELIVERY_ALLOW_PRIMARY_CHECKOUT=1` directive.
