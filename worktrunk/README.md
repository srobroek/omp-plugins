...
Caching is bounded on both sides of the answer it stores. A `git init` or `git clone` is judged before it runs, so the answer that call produces describes a repository about to stop being the truth: the cache is cleared before the decision and again after it, and nothing about the pre-command state survives into the session. A cached *positive* membership is not trusted on its own either. Removals — `wt remove`, `git worktree remove`, another agent's cleanup — are not commands this gate can observe, so a cached worktree path is re-checked against the filesystem before a write is allowed: a linked worktree's `.git` is a file whose `gitdir:` pointer resolves inside that repository's common git directory. Anything the check cannot confirm falls through to `git worktree list`, which is authoritative, so an unusual layout costs a `git` call rather than a false refusal — and an ordinary directory recreated where a worktree used to be is refused.
