...
`delivery_land` and `delivery_cleanup` are judged by the session cwd as well as by the paths they name, because they resolve the receipt, the worktree and the branch from the directory they run in: a payload that names no path still mutates that repository, so both require a live linked worktree as the session cwd. Their declared path parameters — `worktree` on both, `receipt` on `delivery_cleanup` — are judged however they are spelled: a relative value resolves against that same session cwd, so `worktree: "../../canonical"` earns the refusal `worktree: "/abs/canonical"` earns, while a relative value landing inside the worktree is allowed. A key is a path because the tool declares it as one, per tool: the same key on another tool names nothing.

Every other call that resolves to no filesystem path is allowed. A pathless device call, an internal URL, a web URL and a bead id name no working tree this gate can attribute, so there is nothing to contain. This holds for an unenumerated tool too: absence from the tables above costs a tool only the read-only exemption, so it may not carry a canonical path, while a pathless call it makes is still allowed. The two rules compose in that order — the tables decide whether canonical paths are tolerated, the resolved paths decide everything else — and both directions are pinned by tests.
