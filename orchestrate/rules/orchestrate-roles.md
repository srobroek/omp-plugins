...
| `agent:work-reviewer` | `work-reviewer` | MUST review the assigned bead against its acceptance criteria, whether they specify delivered work or a proposed plan; NEVER implement product changes. The implementer implements fixes. |
...
| `agent:operator` | bundled `sonic` | MUST perform mechanical steps; NEVER choose product design or implement feature behavior. The implementer chooses and implements behavior. |
## Delegation Matrix
...
|---|---|---|
| `orchestrator` (lead) | `implementer`, `implementer-high`, `work-reviewer`, `researcher`, `merger`, `shepherd`, `scout`, `sonic` | Dispatches work and plan or acceptance review. |
| `implementer` | `scout`, `sonic`, `researcher` | Delegates investigation, known mechanical work, or scoped research; NEVER `work-reviewer`. |
| `implementer-high` | `scout`, `sonic`, `researcher` | Delegates investigation, known mechanical work, or scoped research; NEVER `work-reviewer`. |
| `work-reviewer` | `scout`, `researcher` | Delegates review investigation or a cited expected-behaviour question; NEVER edit-capable agents. |
| `shepherd` | `scout` | Looks up facts while aggregating and serialising; NEVER implementation or acceptance review. |
| `researcher` | `scout` | Fans out read-only search; NEVER edit-capable agents. |
| `merger`, `scout`, `sonic` | none | Do not spawn children. |
