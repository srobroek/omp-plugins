...
Before dispatching any worker, the lead MUST resolve the package root and run the orchestrate preflight once for the run from that root. Child worktrees MUST NOT rerun this lead-level preflight; they inherit `execution_preflight` and `base_sha`. Record its verdict in the governing run or epic bead metadata as `execution_preflight`, and record the immutable run base commit as `base_sha`. A preflight `FAIL` blocks dispatch: when a failed check reports an applicable `fix`, the lead MUST apply it and rerun the complete preflight with the same `base_sha`; NEVER waive, downgrade, or dispatch on the failed result. If the fix is missing or cannot be applied in the current package/root, the lead MUST stop and report `BLOCKED` with the exact check, detail, and reason. A `skip` reporting that the beads or worktrunk companion is absent also blocks dispatch; other `warn` and `skip` results do not. At run start, the lead MUST claim one governing epic with `bd update EPIC --claim`, write its run and `base_sha` metadata, and read it back with `bd show EPIC --json`; it MUST stop if another live run holds that epic.
...
As each implementer finishes, the lead MUST immediately route its bead to `pool:work-reviewer` and keep the reviewer queue active while other implementers continue. Dispatch every newly ready independent bead in the next role batch; NEVER wait for an entire implementer wave before starting review.
...
A `CHANGES` or `CHANGES_REQUIRED` verdict always requires exactly one fix bead for that failed round, carrying all actionable findings, unless the two-failed-round cap applies. At the cap, the reviewer MUST return `BLOCKED` and the lead MUST stop further work on that bead and create or use an explicit lead `DECISION` gate recording the owner, reason, chosen disposition, and cap evidence; no silent stop, waiver, or additional fix bead is permitted until that gate is resolved.
...
Worker-to-epic integration belongs to the epic orchestrator, never the shepherd. After a work-reviewer returns `APPROVED`, freeze `approved_head` and run `git rev-parse HEAD` in the source worktree; it MUST equal `approved_head` before any merge. Run `git status --porcelain=v1 --untracked-files=no` there and hold if any tracked change exists. Then run `git merge-tree --write-tree EPIC_HEAD WORKER_HEAD` as the conflict preflight. A non-zero result, conflict, or unknown result is a hold: create exactly one fix bead back to the responsible implementer with the preflight evidence, leave the work bead open, and never hand-resolve the conflict. Only a clean preflight permits `wt merge EPIC_BRANCH --no-squash --no-ff --stage tracked` from the source worktree through Bash. `wt merge` rebases the source before merging, so NEVER require its merged head to equal `approved_head`; the default successful merge removes the source worktree, so do not add `--no-remove` because the work bead closes immediately. Capture the epic head before merging, resolve `merged_head` from the merge commit's second parent, and compare the sorted outputs of `git log -p EPIC_HEAD..approved_head | git patch-id --stable` and `git log -p EPIC_HEAD..merged_head | git patch-id --stable`. A patch-id mismatch is a hold that creates one implementer fix bead. On a match, record `approved_head`, `merged_head`, and `merge_sha` on the work bead with `bd update WORK_BEAD --set-metadata approved_head=APPROVED_SHA --set-metadata merged_head=MERGED_SHA --set-metadata merge_sha=MERGE_SHA`, then close it with native `bd close WORK_BEAD --reason ...`.
...
After every worker is integrated and repository verification passes, the orchestrator determines whether an epic-to-default PR exists or is required. It spawns exactly one shepherd for that epic only for that final landing. The shepherd opens or refreshes the epic PR, verifies the exact reviewed head and bot approval, runs `gh pr checks`, calls `delivery_land`, performs native receipt-bead close-out, and calls `delivery_cleanup`. If no epic-to-default PR exists or is required, no shepherd is spawned. In a two-tier run each sub-epic orchestrator integrates its own workers into its sub-epic branch; the root lead then integrates each verified sub-epic branch into the root epic branch with the same approved-head, tracked-clean, merge-tree, patch-id, and `wt merge ROOT_EPIC_BRANCH --no-squash --no-ff --stage tracked` procedure from the sub-epic worktree, and the root lead's one shepherd lands the root epic. A tier1 single-epic run still uses its shepherd for the epic PR.

### Epic-to-default landing method

Before the shepherd invokes `delivery_land`, it MUST read the project's documented landing policy and choose exactly one `merge_method`. A policy that requires a real merge commit or says never to squash selects `merge`; a policy that requires a linear or rebased landing selects `rebase`; a policy that permits squash without requiring either shape selects `squash`. An ambiguous or contradictory policy is a hold, not permission to guess. The shepherd MUST pass the selected `merge_method` to `delivery_land`, require `receipt.proof.evidence.mergeMethod` to match it, and record `merge_method=METHOD` on the epic bead with `bd update EPIC_ID --set-metadata merge_method=METHOD`. The method selection and receipt proof are part of the durable landing evidence.
