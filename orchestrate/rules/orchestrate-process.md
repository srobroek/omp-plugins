...
The main agent, lead, parent, and sub-lead MUST each explicitly instruct every worker to pull its role-routed claim pool. Use this instruction before work begins: `Pull your role-routed ready queue, claim each matching bead, execute it, record evidence, and continue pulling until no ready bead carries your pool alias.` After receiving that instruction, workers MUST continuously run this loop until no ready bead carries their pool alias:
...
4. Execute the bead, record durable evidence, then close it with `bd close ID --reason "EVIDENCE"`, or release it back to its pool with `bd update ID --assignee pool:ROLE --status open --if-assignee ACTOR` and read back its state; workers use only pool-aware CAS release.
5. Return to step 1 after every close or release. Stop only after the pull returns no ready record for the worker's `pool:ROLE` queue and matching `metadata.epic_id`.
...
4. Before any dispatch, create and route AT MOST ONE DAG review bead to the existing `pool:work-reviewer` role; its `--acceptance` criteria MUST cover exactly: dependency correctness, including missing `blocks` edges and cycles; conflict risk, identifying beads that touch the same files or functions by repository inspection rather than titles; wasted or overstated parallelisation against the critical path; and decomposition and overlap, including correctly sized, non-overlapping beads and one owner bead for every shared region with dependents `blocks`-depending on it.
...
If integration conflicts, the shepherd MUST report the conflict to the lead through `write agent://AGENT_ID`, record the refusal on the merge bead, release it with `bd update ID --assignee pool:shepherd --status open --if-assignee ACTOR`, and stop without resolving it. ONLY the lead resolves an integration conflict; the shepherd never reviews its own work, implements product changes, or substitutes for independent review. The lead reruns verification after resolution.
