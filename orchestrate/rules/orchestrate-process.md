...
The main agent, lead, parent, and sub-lead MUST each explicitly instruct every worker to pull its role-routed claim pool. Use this instruction before work begins: `Pull your role-routed ready queue, claim each matching bead, execute it, record evidence, and continue pulling until no ready bead carries your pool alias.` After receiving that instruction, workers MUST continuously run this loop until the bounded pool wait returns a timeout or error:
...
The root MUST NOT call `wait` when nothing it dispatched is still running. After `wait` reports no running jobs, or a worker reports BLOCKED, the root acts instead: it dispatches the next ready bead, re-dispatches or resolves the blocked one, or delivers. In a headless (`omp -p`) run, a `wait` with nothing running returns `Operation aborted` and ends the run with the work unfinished. Two 2026-09-25 benchmark arms ended this way.
