...
4. Give every dispatchable bead its `agent:KIND` routing label, `execution_*` metadata, acceptance and routing context before dispatch; populate git anchors as they become known.
...
10. After work-reviewer approval, create one `agent:shepherd` merge bead with `epic_id`, `source_branch`, exact `source_head`, `target`, `repo`, `review_citation`, and `pr` for PR work; add the work-to-merge dependency and ensure one shepherd per epic is running. Wake an idle shepherd with `write agent://ID` when a new merge bead appears.
