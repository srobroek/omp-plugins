...
10. After work-reviewer approval, create one `pool:shepherd` merge bead with `epic_id`, `source_branch`, exact `source_head`, `target`, `repo`, `review_citation`, and `pr` for PR work; add the work-to-merge dependency and ensure one shepherd per epic is running. Wake an idle shepherd with `write agent://ID` when a new merge bead appears. The shepherd closes only its merge bead and messages the lead with the work bead and merge sha; the lead closes the work bead and source beads after verifying evidence.
