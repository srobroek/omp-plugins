...
3. Record the verdict in the governing run or epic bead metadata as `execution_preflight`, and record the same run SHA as `base_sha`.
4. Dispatch only when `ok` is `true`. A `fail` check blocks dispatch; `warn` and `skip` checks do not.
