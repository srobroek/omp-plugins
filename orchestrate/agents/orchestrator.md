...
2. Build the bead DAG with `bd`. Create every reviewable role bead with `--acceptance`, the `agent:KIND` routing label, the lead-owned epic id in metadata, and `execution_*` metadata before spawning workers. Write git anchors `repo`, `branch`, `base_sha`, `worktree`, `pr`, and `merge_sha` as the worker or integrator learns them. Use `blocks` when research is required before implementation, `discovered-from` for mid-work follow-up, and `related` or `tracks` for non-blocking association. No `--acceptance` means not judgeable.
...
MUST create reviewable beads with `--acceptance` and write `execution_*` metadata before spawn; workers and integrators write git-anchor metadata as known. A bead without acceptance is not judgeable.
