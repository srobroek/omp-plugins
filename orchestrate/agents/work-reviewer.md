...
<procedure>
1. Pull continuously by running the exact command `bd ready --assignee pool:work-reviewer --json`; filter returned records by the lead-owned epic id in metadata, never by parent. If no matching record remains, stop and yield.
...
MUST release unfinished review work with `bd update ID --assignee pool:work-reviewer --status open --if-assignee ACTOR`; use no unguarded release operation.
