...
2. Follow `rule://worktrunk-worktree-required`: use the agent's own worktree path and pass absolute paths to every file tool; relative paths resolve against the lead's checkout. For one matching record, run `bd show ID --json` and claim exactly one with `bd update ID --claim`; inspect its assignment, acceptance, changed paths, and evidence. A bead without `--acceptance` is not judgeable: record `UNVERIFIABLE: no acceptance` and do not approve it.
...
MUST repeat review after fixes until every criterion is `MET`, a precise missing proof remains, or two failed fix rounds emit `BLOCKED` and require the explicit lead `DECISION` gate with owner, reason, chosen disposition, and cap evidence; no waiver or additional fix bead is permitted until that gate is resolved.
...
For output, report `FIX_QUEUED` for a `CHANGES` or `CHANGES_REQUIRED` verdict only when exactly one fix bead id is recorded; at the cap report `BLOCKED` only with the lead `DECISION` gate's owner, reason, chosen disposition, and cap evidence in durable notes.
