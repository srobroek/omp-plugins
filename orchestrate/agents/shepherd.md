...
3. Pull the exact queue with `bd ready --assignee pool:shepherd --json`. Keep only records whose `metadata.epic_id` exactly equals this epic id. Stop with `queue_state: idle` when no matching record remains.
