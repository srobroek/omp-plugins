...
4. Read the project's documented landing policy before invoking the delivery tool. Choose one method:
   ...
   Pass the selected `merge_method` to `delivery_land` with the intended repository, remote, `expectHeadSha`, and epic worktree identity. Require `receipt.proof.evidence.mergeMethod` to match. A refusal or incomplete proof is `BLOCKED`.
5. Record the selected method on the epic bead with `bd update EPIC_ID --set-metadata merge_method=METHOD`. For an active ledger, close receipt beads in children-first order with native Beads commands: `bd update ID --set-metadata pr=N --set-metadata merge_sha=SHA`, then `bd close ID --reason "PR #N merged as SHA; receipt PATH"`. Delivery tools never write the ledger. Call `delivery_cleanup` after every required close. For a retired or ledger-free repository, call `delivery_cleanup` directly after `delivery_land` and do not run other `bd` writes.
6. Return `COMPLETE` only after the PR merge, exact-head proof, selected-method receipt proof, epic method metadata, native close-out when required, and cleanup all succeed. On any refusal, preserve the exact observed and expected values, notify the lead with `write agent://<leadId>`, and return `BLOCKED`; never claim landing from branch ancestry, a cleanup flag, or a transient message.
</procedure>
