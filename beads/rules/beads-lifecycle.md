...
| worker starts | Claim atomically; `status=in_progress` plus assignee record the claim, and the `lease_host`/`lease_pid` anchors prove it still live ([Claiming]rule://beads-core). |
...
`bd gate check` resolves automatic gates (timer, `gh:run`, `gh:pr`, bead).
Force-close only after an explicit human decision recorded on the issue. The
`bd-close-gate` extension refuses `bd close` on a gate bead.
