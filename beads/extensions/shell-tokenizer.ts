...
export type TokenizeOptions = {
	...
	/** Emit opaque stdin-redirection markers while still hiding heredoc bodies. */
	preserveInputRedirects?: boolean;
};
	...
	for (let i = 0; i < command.length; i++) {
		...
		if (ch === "<" && command[i + 1] === "<") {
			...
			if (operator !== null) {
				...
				if (options.preserveInputRedirects) out.push(token(`${descriptor}<<#stdin`));
