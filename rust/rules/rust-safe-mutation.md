...
- Revalidate item freshness with the project's strong version or content identity
  at apply time (for example, a monotonic revision or digest); a changed item
  pauses the plan rather than applying to stale state.
- Normalize paths lexically and `lstat` each component; reject symlink and
  junction traversal unless explicitly enabled per root.
