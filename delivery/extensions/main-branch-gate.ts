 ...
 */
export function findCommitInvocations(
 command: string,
 canChdir?: (path: string) => boolean,
 ...
 for (let i = 0; i < tokens.length; i++) {
  ...
  if (SEPARATOR[token] === true || token === "\n") {
   if (token === "&&" && (failedCd || skipAnd)) skipAnd = true;
   else if (token === ";" || token === "\n" || token === "||") skipAnd = false;
   failedCd = false;
   allowMain = false;
   if (token === "(") {
  ...
  if (ENV_ASSIGNMENT.test(token)) {
   if (token.startsWith(`${ALLOW_ENV}=`)) allowMain = token === `${ALLOW_ENV}=1`;
   continue;
  }
  if (TRANSPARENT_PREFIX[token] === true) continue;
  atCommand = false;
  if (token === "cd") {
   const target = tokens[i + 1];
   const bare = target === undefined || SEPARATOR[target] === true || target === "\n";
   if (bare) {
    if (canChdir && !canChdir(homedir())) {
     failedCd = true;
     continue;
    }
  ...
  if (GIT_COMMANDS[basename(token)] !== true) continue;
  ...
  for (; j < tokens.length; j++) {
   ...
   if (verb === "commit") {
    ...
    if (COMMIT_VALUE_FLAGS[arg] === true) { j++; continue; }
    // Short options may be bundled; a value option consumes the rest or next word.
    if (/^-[^-]/.test(arg)) {
     for (let k = 1; k < arg.length; k++) {
      if (COMMIT_VALUE_FLAGS[`-${arg[k]}`] === true) {
       if (k === arg.length - 1) j++;
       break;
      }
     }
   ...
   if (arg.startsWith("-") && arg !== "-") {
    if (arg.startsWith("-C") && arg.length > 2) {
     ...
     repoDir = repoDir === null || isAbsolute(value) ? value : join(repoDir, value);
     continue;
    }
    const eq = arg.indexOf("=");
    const name = eq === -1 ? arg : arg.slice(0, eq);
    if (verb === null && eq !== -1 && (name === "--git-dir" || name === "--work-tree")) {
     gitArgs.push(arg);
    }
    if (eq === -1 && verb === null && PRE_VERB_VALUE_FLAGS[name] === true) {
     const value = tokens[j + 1];
     if (value !== undefined && SEPARATOR[value] !== true && value !== "\n") {
      if (name === "-C") repoDir = repoDir === null || isAbsolute(value) ? value : join(repoDir, value);
      if (name === "--git-dir" || name === "--work-tree") gitArgs.push(name, value);
      j++;
     }
    }
    continue;
  ...
  i = j - 1;
  if (verb === "commit") out.push({
   repoDir, dryRun, chdir, chdirUnknown,
   ...(allowMain ? { allowMain: true } : {}),
   ...(gitArgs.length ? { gitArgs } : {}),
  });
 }
 return out;
}
 ...
 */
export function currentBranch(cwd: string, gitArgs: string[] = []): string | null {
 const run = injectedRun ?? defaultRun;
 try {
  const result = run(["git", ...gitArgs, "branch", "--show-current"], cwd);
...
export function decideCommit(
 ...
 for (const invocation of findCommitInvocations(command, path => {
  try {
  ...
  if (invocation.dryRun || invocation.allowMain) continue;
  ...
  const branch = currentBranch(target, invocation.gitArgs);
