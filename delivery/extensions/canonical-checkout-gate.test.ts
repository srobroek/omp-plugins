...
import canonicalGate, {
 ...
 setGitRunForTests,
 tokenize,
} from "./canonical-checkout-gate.ts";
...
describe("final registered bypass controls", () => {
	...
	function register(): Handler {
		...
		canonicalGate({ on: (event: string, handler: Handler) => { if (event === "tool_call") handlers.push(handler); } } as never);
