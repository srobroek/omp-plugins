...
MUST GW-6: after a branch lands, cleanup runs through `bd_reconcile`, then `delivery_cleanup`. Never remove a worktree or delete a branch with an ad hoc Git or Worktrunk command. Every agent works in a linked Worktrunk worktree (`rule://worktrunk-isolation-disabled`), and ownership, preconditions, and stopping conditions for that cleanup are in `rule://delivery-worktree-hygiene`.
