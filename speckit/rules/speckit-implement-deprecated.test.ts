...
function condition(): RegExp { const text = fs.readFileSync(RULE, "utf8"); const line = text.split(/\r?\n/).find(l => l.startsWith("condition:")); if (!line) throw new Error("no condition"); const [raw] = JSON.parse(line.slice("condition:".length).trim()) as string[]; const flags = /^\(\?([ims]+)\)/.exec(raw); return flags ? new RegExp(raw.slice(flags[0].length), flags[1]) : new RegExp(raw, "u"); }
