...
|---|---|---|
| Preflight | `references/guidebook.md` | Call `scaffold` with `command: "inspect"`, then `command: "preflight"`, `args: ["--profile", P]`. Show findings and the proposed profile as a table, then `ask` before anything else; a hard blocker changes the options, never the ask. |
